...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: dbPost.title, description: dbPost.excerpt ?? "" };
}
    ...
    <article>
      <section className={`relative overflow-hidden bg-gradient-to-br ${TONE_GRADIENT[cat.tone]}`}>
        <Container className="relative pt-12 pb-16 sm:pt-20 sm:pb-24 max-w-3xl">
          ...
          <Reveal delay={0.1}>
            <h1 className="mt-4 font-display text-2xl sm:text-3xl lg:text-4xl font-semibold leading-[1.1] text-balance">
              {title}
            </h1>
          </Reveal>
          <Reveal delay={0.15}>
            <p className="mt-5 text-base sm:text-lg text-[var(--color-ink-700)] text-pretty">{excerpt}</p>
          </Reveal>
          <Reveal delay={0.2}>
        ...
        <Container className="max-w-3xl">
          <TracingBeam>
            {showDbBody ? (
              <Reveal>
                <div
                  className="prose-content text-[var(--color-ink-800)] leading-relaxed text-base sm:text-lg"
              ...
              <Reveal>
                <div className="prose-content text-[var(--color-ink-800)] leading-relaxed text-base sm:text-lg space-y-4">
              ...
              {related.map((r, idx) => {
                ...
                return (
                  <Reveal key={r.slug} delay={idx * 0.06}>
                    <Link
                      href={`/blog/${r.slug}`}
                      className="group flex flex-col h-full rounded-2xl bg-white p-5 border border-[var(--color-ink-100)] hover:border-[var(--color-primary)] hover:shadow-[var(--shadow-soft)] transition-all"
                    >
                      <span
                      ...
                      <h3 className="mt-3 font-display text-base font-semibold leading-snug group-hover:text-[var(--color-primary)] transition-colors">
                        {r.title}
                      </h3>
